  ...
  return (
    ...
    >
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="w-8 h-[2px] mb-4" style={{ background: plan.highlight ? GRADIENTS.goldIntroBar : RGBA.gold(0.3) }} />

        <h3 className="text-xs font-bold tracking-widest mb-2" style={{ fontFamily: FONTS.title, color: "var(--text-primary)" }}>
          {plan.name}
        </h3>

        <p className="text-[11px] leading-relaxed mb-4" style={{ color: "var(--text-muted)", fontFamily: FONTS.body }}>
          {plan.shortDesc}
        </p>
        ...
        <div className="space-y-1 mb-5 flex-1">
          {plan.features.slice(0, 3).map((f, i) => (
            <div key={i} className="flex items-start gap-1.5">
              ...
              <span className="text-[11px]" style={{ color: "var(--text-body)", fontFamily: FONTS.body }}>{f}</span>
            </div>
          ))}
          {plan.features.length > 3 && (
            <p className="text-[10px] pl-[18px]" style={{ color: COLORS.goldDeep, fontFamily: FONTS.body }}>
              + {plan.features.length - 3} beneficios...
            </p>
          )}
        </div>
        ...
        <div className="flex items-baseline gap-1 mb-4">
          ...
          {plan.period && <span className="text-[10px]" style={{ color: COLORS.goldDeep }}>{plan.period}</span>}
        </div>
    ...
    <section id="planos" className="relative py-24 sm:py-32">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <h2 className="text-base sm:text-xl font-bold tracking-widest leading-snug" style={{ fontFamily: FONTS.title, color: "var(--text-primary)" }}>
