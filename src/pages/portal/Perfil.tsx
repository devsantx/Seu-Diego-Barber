...
import { fetchAgendamentosCliente, mockHistorico } from "../../data/mockData";
...
function fmtDataCompleta(iso?: string | null) {
  if (!iso) return "—";
...
function CampoInfo({ label, valor }: CampoInfoProps) {
  return (
    <div className="py-2 border-b" style={{ borderColor: RGBA.gold(0.07) }}>
      <p className="text-[9px] font-bold tracking-widest" style={{ color: COLORS.gold, fontFamily: FONTS.title }}>
        {label}
      </p>
      <p className="text-[11px] mt-0.5" style={{ color: "var(--text-body)", fontFamily: FONTS.body }}>
        {valor ?? "—"}
      </p>
    </div>
  );
...
export default function Perfil() {
  ...
  useEffect(() => {
    if (!cliente) return;
    fetchAgendamentosCliente(cliente.id).then(setAgendamentos);
  }, [cliente]);
  ...
  const contServicos = mockHistorico.filter(a => a.barbeiro.nome === bTop).reduce<Record<string, number>>((acc, a) => {
    acc[a.servico.nome] = (acc[a.servico.nome] || 0) + 1;
    return acc;
  }, {});
  ...
  const proximoAgendamento = agendamentos.find(item => new Date(`${item.data}T${item.hora}:00`) >= new Date() && item.status === "agendado") ?? null;
  ...
  return (
    <div>
      <motion.h1 className="text-2xl font-bold tracking-widest text-center mb-8" style={{ fontFamily: FONTS.title, color: "var(--text-primary)" }} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        PERFIL
      </motion.h1>
        ...
        <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
              ...
              <div key={a.id} className="flex items-center gap-3 p-3 radius-card" style={{ background: "var(--bg-input)", border: `1px solid ${RGBA.gold(0.07)}` }}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px]" style={{ color: "var(--text-muted)", fontFamily: FONTS.body }}>
                    {fmtData(a.data)} · {a.barbeiro.nome}
                  </p>
                </div>
                {a.comPlano ? (
                  ...
                  <p className="text-[11px] font-semibold flex-shrink-0" style={{ color: COLORS.gold, fontFamily: FONTS.title }}>
                    R$ {a.valor}
                  </p>
                )}
              </div>
          ...
          <div className="flex items-start gap-4 p-4 mb-6 radius-card" style={{ background: "var(--bg-input)", border: `1px solid ${RGBA.gold(0.12)}` }}>
            ...
            <div>
              ...
              <p className="text-[11px] mt-1 leading-relaxed" style={{ color: "var(--text-body)", fontFamily: FONTS.body }}>
                Você já cortou com <strong style={{ color: COLORS.gold }}>{bTop}</strong> <strong style={{ color: COLORS.gold }}>{qtd}</strong> {qtd === 1 ? "vez" : "vezes"} e o seu corte preferido é o <strong style={{ color: COLORS.gold }}>{sTop}</strong>.
              </p>
            </div>
          </div>
          ...
          <div
            ...
            style={{
              ...
              boxShadow: proximoAgendamento ? `0 10px 24px ${RGBA.gold(0.06)}` : "none",
            }}
          >
            {proximoAgendamento ? (
              <>
                <p className="text-[10px] font-bold tracking-[0.2em] mb-2" style={{ color: COLORS.goldDeep, fontFamily: FONTS.title }}>
                  HORARIO JA MARCADO
                </p>
                <p className="text-[11px] font-semibold" style={{ color: "var(--text-primary)", fontFamily: FONTS.body }}>
                  {fmtDataCompleta(proximoAgendamento.data)} as {proximoAgendamento.hora}
                </p>
                <p className="text-[10px] mt-1" style={{ color: "var(--text-body)", fontFamily: FONTS.body }}>
                  Servico: {proximoAgendamento.servico.nome}
                </p>
                <p className="text-[10px] mt-1" style={{ color: "var(--text-body)", fontFamily: FONTS.body }}>
              ...
              <>
                ...
                <p className="text-[11px]" style={{ color: "var(--text-muted)", fontFamily: FONTS.body }}>
                  Assim que um novo agendamento for confirmado, ele aparece aqui.
                </p>
              </>
            )}
          </div>
            ...
            <div className="flex justify-center mb-5">
              <div className="relative cursor-pointer group" onClick={() => fotoRef.current?.click()}>
                <div className="w-20 h-20 rounded-full flex items-center justify-center overflow-hidden" style={{ background: RGBA.gold(0.06), border: `2px solid ${RGBA.gold(0.2)}` }}>
                  {foto ? <img src={foto} alt="Foto" className="w-full h-full object-cover" /> : <span className="text-3xl opacity-30">👤</span>}
                  <div className="absolute inset-0 rounded-full opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity" style={{ background: RGBA.black(0.5) }}>
            ...
            {editando ? (
              <div className="space-y-3">
                {[
                  ...
                  { key: "endereco", label: "ENDERECO", type: "text", ph: "Rua, numero - Bairro" },
                ].map(c => (
                  <div key={c.key}>
                    <label className="block text-[9px] font-bold tracking-widest mb-1" style={{ color: COLORS.goldDark, fontFamily: FONTS.title }}>
                      {c.label}
                    </label>
                    <input
                      type={c.type}
              ...
              <div>
                ...
                <CampoInfo label="ENDERECO" valor={cliente?.endereco} />
              </div>
            )}
          </div>
        </motion.div>
